...
# Time Series loaded from Data_Utils
...
for i in tqdm(range(x, y)):
    try:
        ...
        # Tuner to fine tune learning rate
        ...
        deepar_df = pd.DataFrame(columns = ['original', 'predicted'])
        deepar_df['original'] = df[df.time_idx <= training_cutoff].value.values.tolist() + np.array(actuals.flatten()*std+mean_val).tolist()
        deepar_df['predicted'] = df[df.time_idx <= training_cutoff].value.values.tolist() + np.array(predictions.flatten()*std+mean_val).tolist()
        deepar_df.to_csv('{2}_{3}_{1}_m.csv'.format(training_cutoff, len(actuals), file_name.split('.')[0], 'deepar'))
