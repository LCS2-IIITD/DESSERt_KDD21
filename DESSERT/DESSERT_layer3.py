...
def FindHyperparameters(sigma,sigma1,sigma2,f1,f2,f3,thread_count,seed1,seed2,seed3,Nz,Nx,intialisationFactor,FuturePredDays,no_iter,day,pred_day_start,pred_day_end,window_size,WholeDataMatrix,observation_data,X_col):
  WrongSeedFlag=1
  ...
  A_2= np.random.rand(A_00,A_00) 
  H_0= np.random.rand(H_00,H_01) 
  H_1= np.random.rand(H_10,H_11) 
  H_2= np.random.rand(H_20,H_21) 
  ...
  while(pred_day_end<=num):
            ...
            kalman_mean_a =np.zeros((Nz,cols))  #np.full((Nz,cols), 100)
...
def ForecastForThread(sigma,sigma1,sigma2,f1,f2,f3,thread_count,seed1,seed2,seed3,Nz,Nx,intialisationFactor,FuturePredDays,no_iter,day,pred_day_start,pred_day_end,window_size,WholeDataMatrix,observation_data,X_col):
  rows,cols=WholeDataMatrix.shape
  PredictedMatrix=np.zeros((1,(cols-(window_size+1))))
  ActualMatrix=WholeDataMatrix[0,window_size+1:].reshape((1,(cols-(window_size+1))))
  #Nx should be equal to number of features
  TC=1
  A_00 = Nz
  A_01 = A_00
  A_10 = A_01
...
#Flags for check
  ...
  count1=0
  Uncertanity={}
  Uncertanity['predicted']=[]
  Uncertanity['prev']=[]
  Uncertanity['true']=[]
  Uncertanity['prob']=[]
  Uncertanity['var']=[]
  Uncertanity['trendBinary']=[]
  Uncertanity['logloss']=[]
  flag=0
  while(pred_day_end+1<=(X_col+1)):
            ...
            v1=np.random.normal(0,sigma1,(Nz,1))
            v2 =np.random.normal(0,sigma2,(Nx,1))
            state=(np.dot(D_1_est_a_,kalman_mean_a[:,cols-1].reshape(Nz,1))+v1).reshape(Nz,)
            mean=np.dot(D_2_est_a_,state.reshape(Nz,1))
            new_S=np.dot((np.dot(D_1_est_a_,kalman_covariance_a[cols-1])),D_1_est_a_.T)+Q
            
            observation=ActualMatrix[:,(pred_day_start-window_size):(pred_day_end-window_size)]
            prob=1-norm.cdf(observation[0],mean[0],new_S[0][0])
            if prob[0]==1:
              prob[0]=0.99
            if prob[0]==0:
              prob[0]=0.01
            confidence_score=mean-new_S[0][0]           
            parameter={'Prediction for day':pred_day_start,'Probability':prob,'True:':ActualMatrix[0:1,(pred_day_start-window_size):(pred_day_end-window_size)],'Previous':WholeDataMatrix[0:1,(pred_day_start-1):(pred_day_end-1)]}
            Uncertanity['prev'].append(WholeDataMatrix[:,(pred_day_start-1):(pred_day_end-1)][0][0])
            Uncertanity['true'].append(observation[0][0])
            Uncertanity['prob'].append(prob[0])
            Uncertanity['var'].append(new_S[0][0])
            if observation[0][0]>WholeDataMatrix[:,(pred_day_start-1):(pred_day_end-1)][0][0] :
              flag=1
            Uncertanity['trendBinary'].append(flag)            
            Uncertanity['logloss'].append(-(flag*np.log(prob))[0])
            flag=0
            ...
            pred_day_end=pred_day_start + FuturePredDays
            count1+=1
            Uncertanity['predicted'].append(predicted[0][0])
 
  x=list(range(window_size+1,(X_col+1)))
  Uncertanity['frame']=x
  Uncertanity['Loss']=sum(Uncertanity['logloss'])
  Uncertanity['logLoss Norm']=(Uncertanity['Loss']/len(x))
  for i in range(0,TC):
    fig=plt.figure() 
    plt.errorbar(x, PredictedMatrix[i], Uncertanity['var'],label="predicted")
    plt.plot(x,ActualMatrix[i],label="Actual")
    plt.title('thread'+str(thread_count))
    plt.xlabel('Window Frame')
    plt.ylabel('HateScore')
    plt.legend( loc ="lower right")
    pdfForFeature.savefig(fig)
    fig.savefig('./DSS/'+str(thread_count)+'.pdf')
  PredictedMatrix[np.isnan(PredictedMatrix)]=0
  MAE_List=getMaeForThreads(PredictedMatrix,ActualMatrix,TC)
  RMSE_List=getRMSEForThreads(PredictedMatrix,ActualMatrix,TC)         
  return MAE_List[0],RMSE_List[0],PredictedMatrix,ActualMatrix,Uncertanity
...
pdfForFeature = matplotlib.backends.backend_pdf.PdfPages("./temp.pdf")
counter=1
FinalMetricDict=[]
for Thread in DataList:
  ...
  for values in itertools.product(*map(HyperParameters.get, keys)):
    ...
    Thread_MAE,Thread_RMSE= FindHyperparameters(HP['sigma'],HP['sigma1'],HP['sigma2'],HP['f1'],HP['f2'],HP['f3'],counter,HP['seed1'],HP['seed2'],HP['seed3'],HP['Nz'],5,HP['intialisationFactor'],HP['FuturePredDays'],HP['no_iter'],day,pred_day_start,pred_day_end,window_size,WholeDataMatrix,observation_data,X_col)
  ...
  pred_day_end=pred_day_start +HP['FuturePredDays']
  X_row,X_col,WholeDataMatrix,observation_data = AppendFuturePrediction(Thread,window_size)
  Thread_MAE,Thread_RMSE,PredictedMatrix,ActualMatrix,tempDict=ForecastForThread(HP['sigma'],HP['sigma1'],HP['sigma2'],HP['f1'],HP['f2'],HP['f3'],counter,HP['seed1'],HP['seed2'],HP['seed3'],HP['Nz'],5,HP['intialisationFactor'],HP['FuturePredDays'],HP['no_iter'],day,pred_day_start,pred_day_end,window_size,WholeDataMatrix,observation_data,X_col)
  FinalMetricDict[counter]={'MAE':Thread_MAE,'RMSE':Thread_RMSE}
  uncertanityDictList.append(tempDict)
